...
/*Autor: Jesus Malo, support: dic.malo@gmail.com*/
 ...
 function getScript($option, $persona){
 	/*declaramos parte de la funcion, lo demas se complementa en el switch*/
 	 $funcion ='<script>
 	 			$(".aceptar").click(function(data)
			    {
			   	 if($("#id_persona").val()<=0){
			  		alert("No has seleccionado una persona");
			  		return;
			  	 }
			  	 var persona = {
			  		nombre:$("#nombrePersona").val()+" "+$("#apepat").val()+" "+$("#apemat").val(), 
			  		id_: $("#id_persona").val(),';
	global $db;
 	...
 	{
 		case 'Organizador':
 		 $funcion.='telefono: $("#telefono").val(), 
			  		celular:$("#celular").val(), 
			  		email:$("#email").val()
			  		};
			  	 window.parent.showOrganizador(persona);
			  	 $("#addPersona", window.parent.document).fadeOut(0);
			   });
			   ajustarAltoIframe("iframe_persona",$(document).height());';
 			break;

 		case 'Formador':
 		require_once("../clases/C".$option.".php");		
 		$formador = new CFormador($db);
 		$formador->setIdPersona($persona->getIdPersona());
 		$formador->getFormadorById();
 		 $funcion.='tipo:"'.$formador->getTipoFormador().'",			  	   
			  	};
			  	 window.parent.showFormador(persona);
			  	 $("#addFormador", window.parent.document).fadeOut(0);
			   });
			   ajustarAltoIframe("iframe_formador",$(document).height());';
 			break;
 		...
 		case 'Coordinador': 		
 		 $funcion.='telefono: $("#telefono").val(), 
			  	 ...
			  	 window.parent.showCoordinador(persona);
			  	 $("#addCoordinador", window.parent.document).fadeOut(0);			  	 
			   });
			   ajustarAltoIframe("iframe_coordinador",$(document).height());'; 
 			break;

 		case 'ResponsableCDC':
 		/*BUSCAR persona dentro de los responsables*/
 		 $funcion.='};
			  	 window.parent.showResponsableCDC(persona);
			  	 $("#addResponsableCDC", window.parent.document).fadeOut(0);			  	 
			   });
			   ajustarAltoIframe("iframe_responsableCDC",$(document).height());'; 
 			break;
